...
function playAITurn() {
  ...
  const goteHand = hands.gote;
  for (const type in goteHand) {
    if (goteHand[type] > 0) {
      emptyCells.forEach(cell => {
        // basic nifu check
        if (type === 'P') {
          for (let iy = 0; iy < BOARD_SIZE; iy++) {
            const p = board[iy][cell.x];
            if (p && p.type === 'P' && p.owner === 'gote' && !p.promoted) return;
          }
          if (cell.y === 8) return;
        }
  ...
  // Prefer captures
  const captures = allMoves.filter(m => !m.drop && m.to.type === 'capture');
  const chosenMove = captures.length > 0 ? captures[Math.floor(Math.random() * captures.length)] : allMoves[Math.floor(Math.random() * allMoves.length)];
