(() => {
  ...
  function alcoholDinnerPattern() {
    ...
    const withAlcoholNoCarb = dinners.filter(l => (l.alcoholBeer350 || 0) > 0 && !/(ご飯|パン|麻|米|主食)/.test(l.text || ''));
  ...
  function renderAdvice() {
    ...
    html += block('🍺 晩酌と夕食について', [
      `アルコールは肝臓での糖新生（血糖を保つ働き）を数時間抱えるため、炭水化物ゼロのおつまみ＋ビールという組み合わせは、就寝中〜早朝の低血糖リスクを上げやすい食べ方です。`,
      `飲酒する日は、おかずに少量の根菜・かぼちゃ・全粒粉クラッカーなど「ゆっくり吸収される炭水化物」を少し添えると血糖が安定しやすくなります。`,
      `寝る前にリブレの数値を確認し、90mg/dLを下回っているようならチーズや無糖ヨーグルト＋ナッツなど、少量のたんぱく質＋脂質のスナックを摂ってから休むと夜間低血糖の予防になります。`,
      `リブレアプリの低血糖アラームを80mg/dL程度に設定しておくと、就寝中の低下に早く気づけます。`
    ]);

    // Breakfast/lunch advice
    html += block('🥗 朝食・昼食について', [
      `ゆで卵・キャロットラペ・ささみ蒸しなど、たんぱく質と食物繊維を中心にした朝食は血糖の急上昇を押さえるうえで良い組み合わせです。パンは全粒粉やライ麦を使うと血糖の上がり方がより緩やかになります。`,
      `納豆を先に食べてからお弁当、という食べる順番は食後血糖の急上昇を押さえるのに役立っています。ただし昼食の量が「やや少なめ」とのことなので、夕方の症状が単純な空腹・エネルギー不足から来ていないかも確認してみてください。`,
      `夕方に強い症状が出やすいなら、昼食と夕食の間が空きすぎていないか、15時前後に軽い補食（ナッツ、チーズ、ゆで卵など）を挿むと血糖の落ち込みを防げる場合があります。`
    ]);

    // Menopause-related advice
    html += block(`🌸 更年期・ホルモンの視点（現在: ${menopauseLabel}）`, [
      `50歳・未閉経とのことで、閉経移行期特有のホルモン変動（エストロゲンの変動）によるほてり・動悸・めまい・強い倦怠感が、低血糖の症状と非常によく似ていることが知られています。両方が重なっている可能性もあります。`,
      `婦人科で女性ホルモン（エストラジオール・FSHなど）、内科で甲状腺機能（TSH）・貫血（フェリチン・ヘモグロビン）・HbA1c・空腹時インスリンなどの基本的な血液検査を一度受けておくと、原因の切り分けがしやすくなります。`,
    ...
    html += block('✅ 次にできる小さな一歩', [
      ...
      `婦人科でホルモン検査、内科で血液検査（HbA1c・甲状腺・貫血）を受ける予約を取る。`,
